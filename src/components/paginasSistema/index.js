...
import { getAllPages, addPageFetch, editPageFetch, removePageFetch, getAllPageCategories } from '../../store/fetchActions/accessProfiles';
...
const FORM_INICIAL = {
    titulo: '', path: '', icone: '', categoria: '',
    category_id: '', ordem: 1, ativo: true
};
...
export default function PaginasSistema() {
    ...
    const categoriasAtivas = pageCategories.filter(c => c.ativo).sort((a, b) => {
    ...
    const handleSave = () => {
        const payload = { ...form };
        payload.ordem = Number(payload.ordem || 1);
        if (!payload.category_id) payload.category_id = null;
        if (editId) {
            dispatch(editPageFetch(editId, payload, () => {
                setOpenModal(false);
                dispatch(getAllPages());
            }));
        } else {
            dispatch(addPageFetch(payload, () => {
                setOpenModal(false);
                dispatch(getAllPages());
            }));
        }
            ...
            <Modal keepMounted open={openModal} onClose={() => setOpenModal(false)}>
                <Box sx={modalStyle}>
                    ...
                    <BaseCard title={editId ? 'Editar Página do Sistema' : 'Nova Página do Sistema'}>
                        <Stack spacing={3}>
                            <TextField fullWidth label="Título da Página" name="titulo" value={form.titulo} onChange={handleChange} required inputProps={{ maxLength: 80 }} />
                            <TextField fullWidth label="Path (rota)" name="path" value={form.path} onChange={handleChange} required inputProps={{ maxLength: 120 }} helperText="Caminho da página, ex: /laboratorio/exames" InputProps={{ sx: { fontFamily: 'monospace' } }} />
