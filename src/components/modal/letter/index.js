                    ...
                    <Grid container spacing={0}>
                        <Grid item xs={12} lg={12}>
                            <BaseCard title={letter && letter.id ? "Editar Ofício " : "Enviar Ofício "}>
                                ...
                                <Stack spacing={3}>
                                    <TextField id="sender" label={sender && sender.length > 0 ? `Remetente: ${50 - sender.length} caracteres restantes` : 'Remetente'} variant="outlined" name="sender" value={sender || ''} onChange={changeItem} required inputProps={{ style: { textTransform: "uppercase" }, maxLength: 50 }} />
                                    <TextField id="recipient" label={recipient && recipient.length > 0 ? `Destinatário: ${50 - recipient.length} caracteres restantes` : 'Destinatário'} variant="outlined" name="recipient" value={recipient || ''} onChange={changeItem} required inputProps={{ style: { textTransform: "uppercase" }, maxLength: 50 }} />
                                    <TextField id="subject_matter" label={subject_matter && subject_matter.length > 0 ? `Assunto: ${100 - subject_matter.length} caracteres restantes` : 'Assunto'} variant="outlined" name="subject_matter" multiline value={subject_matter || ''} onChange={changeItem} required inputProps={{ style: { textTransform: "uppercase" }, maxLength: 100 }} />
                                    <TextField id="summary" label={summary && summary.length > 0 ? `Resumo: ${500 - summary.length} caracteres restantes` : 'Resumo'} multiline rows={2} value={summary || ''} name="summary" onChange={changeItem} inputProps={{ style: { textTransform: "uppercase" }, maxLength: 500 }} />
                                    <TextField id="obs" label={obs ? `O modelo gerado pela IA possui ${obs.length} caracteres` : 'Campo destinado a IA'} multiline rows={10} value={obs || ''} disabled name="obs" />
                                </Stack>
