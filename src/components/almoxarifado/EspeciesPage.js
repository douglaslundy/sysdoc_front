...
export default function EspeciesPage() {
  const fields = useMemo(() => ([
    ...
    { name: 'ativo', label: 'Espécie ativa', type: 'switch', defaultValue: true },
  ]), []);

  const columns = useMemo(() => ([
    { key: 'nome', label: 'Espécie', render: (row) => <Typography variant="h6" sx={{ fontWeight: 600 }}>{row.nome}</Typography> },
  ...
  return (
    <CrudPage
      title="Espécies"
      subtitle="Espécies ou tipos de produtos."
      apiPath="/almoxarifado/catalogos/especies"
      fields={fields}
      columns={columns}
