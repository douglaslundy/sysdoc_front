...
export const addClientReport = createAction('ADD_CLIENT_REPORT');
export const clearClientReport = createAction('CLEAR_CLIENT_REPORT');
...
const clientReducer = createReducer(INITIAL_STATE, (builder) => {
  builder
    ...
    .addCase(editClient, (state, action) => {
      state.clients = [
        action.payload,
        ...state.clients.filter(cli => cli.id !== action.payload.id)
      ];
    })
    ...
    .addCase(addClientReport, (state, action) => {
      state.clientReport = action.payload;
    })

    .addCase(clearClientReport, (state) => {
